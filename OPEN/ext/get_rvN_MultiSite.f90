...
!*  argument of the periastron and time of periastron. vsys is an array with the
!*  systematic velocities for each observatory.
!*
!*  This routine works for NP planets and NOB observatories
subroutine get_rvN(time, P, K, ecc, omega, t0, vsys, observ, vel, nt, np, nob)
    implicit none
    
!f2py intent(in) time(nt)
!f2py intent(inout) vel(nt)
!f2py intent(in) observ(nt)
!f2py intent(in) P(np)
!f2py intent(in) K(np)
!f2py intent(in) ecc(np)
!f2py intent(in) omega(np)
!f2py intent(in) t0(np)
!f2py intent(in) vsys(nob)
!f2py intent(hide) nt
!f2py intent(hide) np
!f2py intent(hide) nob

! Input arguments
    integer nt, np, nob ! number of observations, planets, observatories
    real (kind=8), dimension(nt) :: time, vel, observ
    real (kind=8), dimension(np) :: p, k, ecc, omega, t0
    real (kind=8), dimension(nob) :: vsys
